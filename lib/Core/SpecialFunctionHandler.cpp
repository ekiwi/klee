...
#define addDNR(name, handler) { name, \
  ...
  add("klee_dump_expr", handleDumpExpr, false),
...
SpecialFunctionHandler::SpecialFunctionHandler(Executor &_executor) 
  : executor(_executor), expr_dump_ctr() {}
...
class ExprDumpPrinter : private ExprSMTLIBPrinter {
public:
  ExprDumpPrinter(llvm::raw_ostream &output, ExecutionState& state)
    : ExprSMTLIBPrinter(),
      query(state.constraints, klee::ConstantExpr::alloc(0, Expr::Bool))
  {
    setOutput(output);
    setQuery(query);
  }
  ...
  void printSetLogic() {
    ExprSMTLIBPrinter::printSetLogic();
  }

  void printArrayDeclarations() {
    ExprSMTLIBPrinter::printArrayDeclarations();
  }

  void printMachineReadableQuery() {
    ExprSMTLIBPrinter::printMachineReadableQuery();
  }

  void printPathConstraints() {
    ref<Expr> queryAssert = Expr::createIsZero( klee::ConstantExpr::alloc(0, Expr::Bool));
  ...
  void printExpression(const ref<Expr> &e) {
    ExprSMTLIBPrinter::SMTLIB_SORT expectedSort = ExprSMTLIBPrinter::getSort(e);
    ExprSMTLIBPrinter::printExpression(e, expectedSort);
  }

  void printSort(const ref<Expr> &e) {
    const auto sort = ExprSMTLIBPrinter::getSort(e);
    if (sort == ExprSMTLIBPrinter::SORT_BITVECTOR) {
      *p << "(_ BitVec " << e->getWidth() <<")";
    } else {
      *p << "Bool";
    }
  }
...
static constexpr bool DumpAsSmtNotUclid = false;

void SpecialFunctionHandler::handleDumpExpr(ExecutionState &state,
                                  KInstruction *target,
                                  std::vector<ref<Expr> > &arguments) {
  assert(arguments.size()==2 &&
         "invalid number of arguments to klee_dump_expr");

  std::string expr_name = readStringAtAddress(state, arguments[0]);
  const auto ii = expr_dump_ctr[expr_name] + 1;
  expr_dump_ctr[expr_name] = ii;
  const auto filename = expr_name + std::string("_") + std::to_string(ii);

  if(DumpAsSmtNotUclid) {
    auto f = this->executor.interpreterHandler->openOutputFile(filename + std::string(".smt2"));
    ExprDumpPrinter printer(*f, state);

    *f << "; generated by a call to `klee_dump_expr(.., " << expr_name << ")`" << "\n";
    //printer.printSetLogic();
    printer.printArrayDeclarations();
    *f << "; dumped expression" << "\n";
    *f << "(define-fun |" << expr_name << "_" << ii << "| () ";
    printer.printSort(arguments[1]);
    *f << " ";
    printer.printExpression(arguments[1]);
    *f << ")\n";
    *f << "; path condition under which dumped expression is valid (use in ITE)" << "\n";
    *f << "(define-fun |path_cond_" << ii << "| () Bool ";
    printer.printPathConstraints();
    *f << ")\n";
    f->flush();
    f->close();
  } else {
    auto f = this->executor.interpreterHandler->openOutputFile(filename + std::string(".smt2"));
    Query q(state.constraints, arguments[1]);
    ExprUCLID5Printer printer(q, *f);

    *f << "// generated by a call to `klee_dump_expr(.., " << expr_name << ")`" << "\n";
    printer.printArrayDeclarations();
    *f << "// path condition under which dumped expression is valid (use in ITE)" << "\n";
    printer.printPathConditions(std::string("path_cond_") + std::to_string(ii));
    *f << "// dumped expression" << "\n";
    printer.printExpr(expr_name + std::string(" ") + std::to_string(ii));
    f->flush();
    f->close();
  }
}
